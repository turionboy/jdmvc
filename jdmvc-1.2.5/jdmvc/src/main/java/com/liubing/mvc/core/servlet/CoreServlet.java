...
/**
 * 框架核心控制层 Servlet implementation class CoreServlert
 */
public class CoreServlet extends HttpServlet {
	...
	/**
	 * MVC 核心核心控制层， 判断http是ajax请求，那么直接print参数 如果不是直接跳转
	 */
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		Object result = "";
		try {
			ConcurrentHashMap<String, Object> map = coreMvcService
					.isRouteInfos(routeInfos, pathInfo, method);

			if (Boolean.parseBoolean(map.get("isRouteInfos").toString())) {
				RouteInfo routeInfo = (RouteInfo) map.get("routeInfo");
				result = (String) coreMvcService.methodInvoke(routeInfo,
						isSupportSpring);
				if (MvcPageUtil.isAjaxRequest(MvcPageContextUtil.getRequest())
						&& !StringUtil.isNullOrEmpty(result)) {// ajax请求
					viewRender.renderResult(MvcPageContextUtil.getRequest(),
							MvcPageContextUtil.getResponse(), result,
							routeInfo.getReturnType());
				} else if (!StringUtil.isNullOrEmpty(result)) {// 页面跳转
					viewRender.renderPage(MvcPageContextUtil.getRequest(),
							MvcPageContextUtil.getResponse(), result + "");
				}
			}
		} catch (Exception e) {
	...
	public void init() throws ServletException {
		...
		isSupportSpring = Boolean
				.parseBoolean(getInitParameter("isSupportSpring"));
		if (StringUtil.isNullOrEmpty(getInitParameter("viewRender"))) {
			...
			try {
				Class<?> clazz = classLoader
						.loadClass(getInitParameter("viewRender"));
		...
		cache = new CacheImpl<String, ConcurrentLinkedQueue<RouteInfo>>(
				routeInfos.size());
	...
	private void handleActionMethodException(HttpServletRequest request,
