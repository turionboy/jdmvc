 ...
 * 
 * <p>
 * Description: 扫描指定包（包括jar）下的class文件
 * </p>
 */
public class ScanClassPathUtil {
	 ...
	 * 
	 * <br/>
	 * Description:无参构造器，默认是排除内部类、并搜索符合规则
	 */
	 ...
	 * 
	 * <br/>
	 * Description:有参数据的构造器
	 * 
	 * 
	 * @param excludeInner
	 *            :是否排除内部类 true->是 false->否<br>
	 * @param checkInOrEx
	 *            ：过滤规则适用情况 true—>搜索符合规则的 false->排除符合规则的<br>
	 * @param classFilters
	 *            ：自定义过滤规则，如果是null或者空，即全部符合不过滤
	 */
	public ScanClassPathUtil(Boolean excludeInner, Boolean checkInOrEx,
		...
		try {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol) || "vfs".equals(protocol)) {
					...
					scanPackageClassesByJar(packageName, url, recursive,
							classes);
				}
			}
		} catch (IOException e) {
	 ...
	 * 
	 * <br/>
	 * Description:以jar的方式扫描包下的所有Class文件
	 * 
	 * 
	 * @param basePackage
	 * @param url
	 ...
	 * @param classes
	 */
	private void scanPackageClassesByJar(String basePackage, URL url,
	 ...
	 * 
	 * <br/>
	 * Description:以文件的方式扫描包下的所有Class文件
	 * 
	 * 
	 * @param classes
	 * @param packageName
	 ...
	 * @param recursive
	 */
	private void scanPackageClassesByFile(Set<Class<?>> classes,
		...
		for (File file : dirfiles) {
			if (file.isDirectory()) {
				scanPackageClassesByFile(classes,
	...
	// classFilters.clear();
	// classFilters.add("*");
	// ScanClassPathUtil handler = new
	// ScanClassPathUtil(false,true,classFilters);
	...
	//
	// Set<Class<?>> calssList =
	// handler.getPkgClassAll(".com.yespme.framework.",true);
